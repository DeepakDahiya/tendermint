...
import (
	...
	tmstore "github.com/deepakdahiya/tendermint/proto/tendermint/store"
...
func TestBlockStoreSaveLoadBlock(t *testing.T) {
	...
	header1 := types.Header{
		Version:         tmversion.Consensus{Block: uint64(version.BlockProtocol)},
		Height:          1,
		ChainID:         "block_test",
		Time:            tmtime.Now(),
...
func TestLoadBlockMeta(t *testing.T) {
	...
	meta := &types.BlockMeta{Header: types.Header{
		Version: tmversion.Consensus{
			Block: uint64(version.BlockProtocol), App: 0}, Height: 1, ProposerAddress: tmrand.Bytes(crypto.AddressSize)}}
	pbm := meta.ToProto()
